...
class SIP_Reviews_Shortcode_WC_Admin {
  ...
  public function sip_rswc_config_menu() {
    ...
    if ( ! empty( $parent ) ) {
      add_submenu_page( $parent , 'Reviews Shortcode', 'Reviews Shortcode', 'manage_options', 'sip-reviews-shortcode-settings', array( $this, 'sip_rswc_settings_page_ui' ) );
    } else {
  ...
  public function sip_rswc_admin_assets() {
    wp_register_style( 'sip_rswc_custom_wp_admin_css', esc_url( SIP_RSWC_URL .   '/admin/assets/css/admin.css', false, '1.0.0' ) );
    wp_enqueue_style( 'sip_rswc_custom_wp_admin_css' );
  }
  ...
  <div class="sip-rs-wrap">
    <div class="sip-rs-settings-header">
      <div class="sip-rs-header-left">
        ...
        <h1 class="rs"><?php echo SIP_RSWC_NAME ; ?></h1>
        <h3>Display product reviews in any post/page with a shortcode.</h3>
      </div>
      <div class="sip-rs-header-right">
        <img class="sip-rs-header-img" src="<?php echo SIP_RSWC_URL ?>admin/assets/images/icon-reviews-shortcode.png">
      </div>
    </div>
    <div class="sip-container">
      <h2 class="nav-tab-wrapper">
        <a class="nav-tab<?php if ( !isset( $_GET['action'] ) ) echo ' nav-tab-active'; ?>" href="admin.php?page=sip-reviews-shortcode-settings"><?php _e( 'Settings', 'sip-social-proof' ); ?></a>
        <a class="nav-tab<?php if ( isset( $_GET['action'] ) && 'usage' == $_GET['action'] ) echo ' nav-tab-active'; ?>" href="admin.php?page=sip-reviews-shortcode-settings&amp;action=usage"><?php _e( 'Usage', 'sip-social-proof' ); ?></a>
      </h2>
      <?php
        if ( ! isset( $_GET['action'] ) ) { 
          sip_rswc_settings_page_ui();
        } elseif ( 'usage' == $_GET['action'] ) { 
          include("ui/usage.php");
        } 
      ?>
    </div><!-- .container -->
  </div> 
